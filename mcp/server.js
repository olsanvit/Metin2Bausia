...
function createMcpServer() {
    ...
    return {
      content: [{
        ...
        text: JSON.stringify({
          ...
          supportsFileOps: true,          // MT2 Bausia: file save/read/compare
          supportsProtoExport: true,      // MT2 Bausia: item_proto / mob_proto export
          supportsApprovalWorkflow: true  // MT2 Bausia: content approval
        }, null, 2)
      }]
    };
  });
  ...
  // ══════════════════════════════════════════════════════════════
  // MT2 BAUSIA — File Operation Tools
  // Soubory jsou uloženy v MT2_FILES_DIR (mount: /app/mt2files)
  // ══════════════════════════════════════════════════════════════

  const MT2_FILES_DIR = process.env.MT2_FILES_DIR || "/app/mt2files";

  // ── list_files ──────────────────────────────────────────────────────────────
  wrapTool("list_files", "List files in an MT2 server directory (relative to MT2_FILES_DIR).", {
    dir: z.string().optional().default(""),
    pattern: z.string().optional()
  }, async ({ dir, pattern }) => {
    const target = path.join(MT2_FILES_DIR, dir.replace(/\.\./g, ""));
    let files;
    try {
      files = await fs.readdir(target, { withFileTypes: true });
    } catch (e) {
      return { content: [{ type: "text", text: JSON.stringify({ error: e.message, path: target }) }] };
    }
    const result = files
      .filter(f => !pattern || f.name.includes(pattern))
      .map(f => ({ name: f.name, type: f.isDirectory() ? "dir" : "file" }));
    return { content: [{ type: "text", text: JSON.stringify({ path: target, entries: result }, null, 2) }] };
  });

  // ── read_file ───────────────────────────────────────────────────────────────
  wrapTool("read_file", "Read text content of an MT2 server file.", {
    filePath: z.string()
  }, async ({ filePath }) => {
    const safe = path.join(MT2_FILES_DIR, filePath.replace(/\.\./g, ""));
    try {
      const content = await fs.readFile(safe, "utf8");
      const stat = await fs.stat(safe);
      return { content: [{ type: "text", text: JSON.stringify({ path: safe, size: stat.size, content }, null, 2) }] };
    } catch (e) {
      return { content: [{ type: "text", text: JSON.stringify({ error: e.message, path: safe }) }] };
    }
  });

  // ── save_file ───────────────────────────────────────────────────────────────
  wrapTool("save_file", "Save (create or overwrite) a text file in MT2 server files. Creates parent dirs.", {
    filePath: z.string(),
    content: z.string(),
    encoding: z.enum(["utf8", "latin1"]).optional().default("utf8")
  }, async ({ filePath, content, encoding }) => {
    const safe = path.join(MT2_FILES_DIR, filePath.replace(/\.\./g, ""));
    await fs.mkdir(path.dirname(safe), { recursive: true });
    // Backup předchozí verze
    let backed = false;
    try {
      const prev = await fs.readFile(safe, encoding);
      const backupPath = safe + ".bak." + Date.now();
      await fs.writeFile(backupPath, prev, encoding);
      backed = true;
    } catch {}
    await fs.writeFile(safe, content, encoding);
    const stat = await fs.stat(safe);
    return { content: [{ type: "text", text: JSON.stringify({ saved: true, path: safe, size: stat.size, backupCreated: backed }) }] };
  });

  // ── update_file ─────────────────────────────────────────────────────────────
  wrapTool("update_file", "Update part of a text file — replace oldText with newText.", {
    filePath: z.string(),
    oldText: z.string(),
    newText: z.string()
    ...
    const safe = path.join(MT2_FILES_DIR, filePath.replace(/\.\./g, ""));
    let current;
    try { current = await fs.readFile(safe, "utf8"); } catch (e) {
      return { content: [{ type: "text", text: JSON.stringify({ error: "file_not_found", path: safe }) }] };
    }
    if (!current.includes(oldText)) {
      return { content: [{ type: "text", text: JSON.stringify({ error: "old_text_not_found", hint: "Use save_file for full overwrite" }) }] };
    }
    const updated = current.replace(oldText, newText);
    await fs.writeFile(safe, updated, "utf8");
    return { content: [{ type: "text", text: JSON.stringify({ updated: true, path: safe }) }] };
  });

  // ── compare_files ───────────────────────────────────────────────────────────
  wrapTool("compare_files", "Compare two text files line-by-line and return diff summary.", {
    filePathA: z.string(),
    filePathB: z.string()
  }, async ({ filePathA, filePathB }) => {
    const safeA = path.join(MT2_FILES_DIR, filePathA.replace(/\.\./g, ""));
    const safeB = path.join(MT2_FILES_DIR, filePathB.replace(/\.\./g, ""));
    let a, b;
    try { a = (await fs.readFile(safeA, "utf8")).split("\n"); } catch (e) {
      return { content: [{ type: "text", text: JSON.stringify({ error: `fileA not readable: ${e.message}` }) }] };
    }
    try { b = (await fs.readFile(safeB, "utf8")).split("\n"); } catch (e) {
      return { content: [{ type: "text", text: JSON.stringify({ error: `fileB not readable: ${e.message}` }) }] };
    }
    const maxLen = Math.max(a.length, b.length);
    const diffs = [];
    for (let i = 0; i < maxLen; i++) {
      if (a[i] !== b[i]) diffs.push({ line: i + 1, a: a[i] ?? "(missing)", b: b[i] ?? "(missing)" });
    }
    return { content: [{ type: "text", text: JSON.stringify({
      fileA: safeA, linesA: a.length,
      fileB: safeB, linesB: b.length,
      diffCount: diffs.length,
      identical: diffs.length === 0,
      diffs: diffs.slice(0, 200)  // max 200 diff lines
    }, null, 2) }] };
  });
  ...
  // ── get_pending_content ─────────────────────────────────────────────────────
  wrapTool("get_pending_content", "Get content items waiting for admin approval.", {
    entityType: z.enum(["Items","Mobs","Maps","Skills","Quests","Systems","ContentImages","all"]).optional().default("all"),
    limit: z.number().optional().default(50)
  }, async ({ entityType, limit }) => {
    const tables = entityType === "all"
      ? ["Items","Mobs","Maps","Skills","Quests","Systems"]
      : [entityType];
    const result = {};
    for (const t of tables) {
      try {
        const r = await pool.query(
          `SELECT "Guid","Vnum","Name","ContentStatus","SourceUrl","ConfidenceScore","CreatedAt"
           FROM "${t}" WHERE "ContentStatus"='pending' AND "IsDeleted"=false
           ORDER BY "ConfidenceScore" DESC NULLS LAST LIMIT $1`, [limit]
        );
        result[t] = r.rows;
      } catch {}
    }
    const total = Object.values(result).reduce((s, r) => s + r.length, 0);
    return { content: [{ type: "text", text: JSON.stringify({ total, byType: result }, null, 2) }] };
  });

  // ── approve_content ─────────────────────────────────────────────────────────
  wrapTool("approve_content", "Approve a content item (set ContentStatus='approved'). Logs to ApprovalLog.", {
    entityTable: z.enum(["Items","Mobs","Maps","Skills","Quests","Systems","ContentImages"]),
    entityGuid: z.string(),
    operator: z.string().optional().default("admin"),
    note: z.string().optional()
  }, async ({ entityTable, entityGuid, operator, note }) => {
    const now = new Date().toISOString();
    const upd = await pool.query(
      `UPDATE "${entityTable}" SET "ContentStatus"='approved', "ApprovedAt"=$1, "ApprovedBy"=$2, "UpdatedAt"=$1
       WHERE "Guid"=$3 AND "IsDeleted"=false RETURNING "Guid","Vnum","Name","ContentStatus"`,
      [now, operator, entityGuid]
    );
    if (upd.rows.length === 0) return { content: [{ type: "text", text: JSON.stringify({ error: "not_found_or_deleted" }) }] };
    const row = upd.rows[0];
    await pool.query(
      `INSERT INTO "ApprovalLog" ("EntityTable","EntityGuid","EntityVnum","EntityName","Action","Operator","Reason","PrevStatus","NewStatus")
       VALUES ($1,$2,$3,$4,'approved',$5,$6,'pending','approved')`,
      [entityTable, entityGuid, row.Vnum, row.Name, operator, note || ""]
    );
    return { content: [{ type: "text", text: JSON.stringify({ approved: true, entity: row }) }] };
  });

  // ── reject_content ──────────────────────────────────────────────────────────
  wrapTool("reject_content", "Reject a content item (set ContentStatus='rejected'). Logs to ApprovalLog.", {
    entityTable: z.enum(["Items","Mobs","Maps","Skills","Quests","Systems","ContentImages"]),
    entityGuid: z.string(),
    operator: z.string().optional().default("admin"),
    reason: z.string().optional()
  }, async ({ entityTable, entityGuid, operator, reason }) => {
    const now = new Date().toISOString();
    const upd = await pool.query(
      `UPDATE "${entityTable}" SET "ContentStatus"='rejected', "RejectedAt"=$1, "RejectedReason"=$2, "UpdatedAt"=$1
       WHERE "Guid"=$3 AND "IsDeleted"=false RETURNING "Guid","Vnum","Name"`,
      [now, reason || "", entityGuid]
    );
    if (upd.rows.length === 0) return { content: [{ type: "text", text: JSON.stringify({ error: "not_found_or_deleted" }) }] };
    const row = upd.rows[0];
    await pool.query(
      `INSERT INTO "ApprovalLog" ("EntityTable","EntityGuid","EntityVnum","EntityName","Action","Operator","Reason","PrevStatus","NewStatus")
       VALUES ($1,$2,$3,$4,'rejected',$5,$6,'pending','rejected')`,
      [entityTable, entityGuid, row.Vnum, row.Name, operator, reason || ""]
    );
    return { content: [{ type: "text", text: JSON.stringify({ rejected: true, entity: row }) }] };
  });
